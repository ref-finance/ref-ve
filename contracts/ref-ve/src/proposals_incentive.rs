...
impl Proposal {
    ...
    pub fn claim_reward(&mut self, account_votes_amount: Balance) -> Option<(AccountId, Balance)> {
        ...
        if let Some(incentive) = &mut self.incentive {
            let ret = incentive.calc_reward(self.participants, votes_total_amount, account_votes_amount);
            incentive.claimed_amount += ret.1;
            Some(ret)
        } else {
            None
        }
...
impl Contract {
    pub fn internal_deposit_reward(&mut self, proposal_id: u32, token_id: &AccountId, amount: Balance) -> (Balance, Timestamp) {
