...
mod tests {
    ...
    fn event_proposal_create() {
        ...
        Event::ProposalCreate { proposer_id, proposal_id, lock_near, kind, start_at, duration_sec, incentive_detail }.emit();
        assert_eq!(
            test_utils::get_logs()[0],
            r#"EVENT_JSON:{"standard":"ref-ve","version":"1.0.0","event":"proposal_create","data":[{"proposer_id":"alice","proposal_id":0,"lock_near":"0","kind":"FarmingReward { farm_list: [\"ref<>celo\", \"usn<>usdt\"], num_portions: 2 }","start_at":1000,"duration_sec":500,"incentive_detail":"Some((AccountId(\"ref\"), Evenly))"}]}"#
        );
    }
    ...
    #[test]
    fn event_lpt_withdraw_slashed() {
        let owner_id = &alice();
        let withdraw_amount = &U128(100);
        let success = true;
        Event::LptWithdrawSlashed { owner_id, withdraw_amount, success }.emit();
        assert_eq!(
            test_utils::get_logs()[0],
            r#"EVENT_JSON:{"standard":"ref-ve","version":"1.0.0","event":"lpt_withdraw_slashed","data":[{"owner_id":"alice","withdraw_amount":"100","success":true}]}"#
        );
    }
