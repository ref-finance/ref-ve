...
ORIG=releases/ref_ve_release.wasm
DEST=res/ref_ve_release.wasm
...
echo 'Using docker to comparing ref_ve_release.wasm in releases folder with the one (if exist) in res folder ...'
...
docker cp releases/ref_ve_release.wasm ${CONTAINER}:/host/release.wasm
ls -l res | grep ref_ve_release.wasm > /dev/null && docker cp res/ref_ve_release.wasm ${CONTAINER}:/host/build.wasm
